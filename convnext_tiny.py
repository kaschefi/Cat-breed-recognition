...
def main():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)), # Shifts and zooms the image randomly
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3), # Stronger color shifts
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        transforms.RandomErasing(p=0.2) # Randomly blocks out parts of the image so the model can't rely on a single feature (like an ear shape)
    ])
    ...
    # Changed num_workers to 2 for Colab stability
    ...
    precision, recall, f1 = evaluate_and_generate_metrics(model, val_loader, DEVICE, CLASS_NAMES, MODEL_NAME, OUTPUT_DIR)
    ...
    print(f"\nAll tasks finished successfully. Check the '{OUTPUT_DIR}/' folder in your Google Drive.")
